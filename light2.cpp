...
//    glUniform3f(loc+1, m.diffuse.x,m.diffuse.y,m.diffuse.z);
    //glUniform3f(loc+1, m.specular.x,m.specular.y,m.specular.z);
    glUniform1f(loc+2,m.shininess);
}
...
void setLight(const light &l,const GLint loc){
    ...
    glUniform3f(loc+4, l.fading.x,l.fading.y,l.fading.z);
...
//    glDisable(GL_DEPTH_TEST);
    ...
    shader cubeSh("../assets/light/l3.vertex.glsl", "../assets/light/l3.fragment.glsl");
    ...
    glBindAttribLocation(cubeSh.getIdProgram(), attrib_texCoords, "a_texCoords");
    cubeSh.link();
    ...
    glVertexAttribPointer(attrib_normal, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 8, (GLvoid*)(3*sizeof (GLfloat)));
    glEnableVertexAttribArray(attrib_normal);

    //attribute of texture coords
    glVertexAttribPointer(attrib_texCoords, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 8, (GLvoid*)(6*sizeof (GLfloat)));
    ...
    glm::vec3 lightPos(-1,0,4);


    glm::mat4 projection = glm::perspective(45.f,(GLfloat)(WIDTH)/(GLfloat)(HEIGHT),0.1f,100.f);
    glm::mat4 view = glm::lookAt(glm::vec3(-3,0,2),glm::vec3(0,0,0),glm::vec3(0,1,0));
    ...
    l1.u_mat  = glGetUniformLocation(cubeSh.getIdProgram(),"u_mat.diffuse");
    ...
    texture2D t1("../assets/container2.png");
    texture2D t2("../assets/container2_specular.png");
    ...
    l.fading = glm::vec3(1,0.09f,0.032f);
...
//        l.ambient = 0.2f* l.diffuse;
        ...
        glUniform1i(l1.u_mat, 0);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, t2.getId());
        glUniform1i(l1.u_mat+1, 1);
        glUniformMatrix4fv(l1.u_view,1,GL_FALSE,glm::value_ptr(view));
        glUniformMatrix4fv(l1.u_projection, 1, GL_FALSE, glm::value_ptr(projection));

        glBindVertexArray(cubeVAO);
        for (int i=0;i<10;i++) {
            modelCube = glm::translate(IDENTITY,glm::vec3(2*i,0,0));
            norTransform = glm::mat3(glm::transpose(glm::inverse(view * modelCube)));
            glUniformMatrix4fv(l1.u_model, 1, GL_FALSE, glm::value_ptr(modelCube));
            glUniformMatrix3fv(l1.u_norTransform, 1, GL_FALSE, glm::value_ptr(norTransform));
